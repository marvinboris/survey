...
export default function Button({ color = 'primary', size = '', icon, children, status, onClick, className }: ButtonProps) {
    ...
    return <button onClick={onClick} className={classNames(`btn btn-${color} relative group`, size === 'sm' ? 'h-10 px-4 space-x-2' : 'px-6 space-x-[26px]', className || '')}>
        <span className={classNames(size === 'sm' ? '' : 'font-semibold inline-block text-lg p-2.5')}>
            {status ? <>
